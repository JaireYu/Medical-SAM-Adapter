...
class ICON(Dataset):
    def __init__(self, args, data_path , transform = None, transform_msk = None, mode = 'Training'):
        if mode == "Training":
            self.image_root = os.path.join(data_path, "train", "images")
            anno_file = os.path.join(data_path, "train", "annotations.json")
            self.annos = json.load(open(anno_file, 'r'))["annotations"]
            self.images = json.load(open(anno_file, 'r'))["images"]
            self.id_to_image_path = {}
            for image in self.images:
                self.id_to_image_path[image["id"]] = os.path.join(data_path, image["file_name"])
        elif mode == "Test":
    ...
    def __getitem__(self, index):
        ...
        img_path = self.id_to_image_path[image_id]
